...
const Utils = {
    async fetch(endpoint, options = {}) {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(data.detail || data.error || data.message || 'Erro na requisição');
            }
...
const Auth = {
    ...
    setupTabs() {
        $$('.auth-tab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                const targetForm = document.getElementById(target) || document.getElementById(`${target}-form`);
                if (targetForm) {
                    targetForm.classList.add('active');
                }
                const loginError = $('#login-error');
                const registerError = $('#register-error') || $('#registro-error');
    ...
    setupForms() {
        const loginForm = $('#login-form') || $('#form-login');
        const registerForm = $('#register-form') || $('#form-registro');
        ...
        if (registerForm) registerForm.addEventListener('submit', async (e) => {
            ...
            const nome = ($('#register-nome') || $('#registro-nome'))?.value?.trim();
            const email = ($('#register-email') || $('#registro-email'))?.value?.trim();
            const senha = ($('#register-senha') || $('#registro-senha'))?.value;
            ...
            if (senha.length < 6) {
                const registerError = $('#register-error') || $('#registro-error');
                if (registerError) registerError.textContent = 'Senha deve ter no mínimo 6 caracteres';
                return;
            }
    ...
    async checkSession() {
        ...
        if (stored) {
            try {
                ...
                const isValid = verifyData.valid === true || verifyData.id || verifyData.user_id;
                if (isValid) {
